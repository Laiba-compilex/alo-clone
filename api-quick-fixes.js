...
class APIQuickFixes {
  ...
  generateFixReport() {
    const report = {
      ...
      recommended_fixes: this.fixes.filter((f) => f.status === "recommended")
        .length,
      fixes: this.fixes,
    };
    ...
    console.log(
      `✅ Applied: ${report.applied_fixes}/${report.total_fixes} fixes`
    );

    if (report.recommended_fixes > 0) {
      console.log(
        `💡 Recommended: ${report.recommended_fixes} additional fixes`
      );
    }
